...
<body>
	...
	<div id="container">
		...
		<div id="content">
			...
			<%-- 게시글 작성 영역 --%>
			<form method="post" action="${pageContext.request.contextPath }/jsp/board_edit_ok.do" id="writeForm">
				<%-- action 페이지에서 사용할 WHERE 조건값을 hidden 필드로 숨겨서 전송한다. --%>
				<input type="hidden" name="bbs_board_id" value="${output.bbs_board_id }" />
					...
					<table class="table table-striped">
						<thead>
							<tr>
								...
								<td>
									<input type="text" name=head id="board_write head" class="board_write" value="${output.head }" />
								</td>
							</tr>
						</thead>
				...
				<%-- 테이블 영역 - BootStrap 사용 끝 --%>

				<%-- 게시글 작성 본문 영역 --%>
				<div class="text_content" id="text_content">
					<textarea id="write_content" class="ckeditor" name="content">
					<c:out value="${output.content}" escapeXml = "true" />
					</textarea>
				</div>
				<%-- 게시글 작성 본문 영역 --%>
